...
import { AlertCircle, CheckCircle2, Sparkles, RefreshCw, Trash2, BookOpen } from "lucide-react"
...
export default function WHQuestionsValidator() {
  ...
  const validateWHQuestion = (question: string): boolean => {
    ...
    if (normalizedQuestion.charAt(0) !== normalizedQuestion.charAt(0).toUpperCase()) return false
    ...
    const auxiliaryPattern = /^(what|where|when|why|who|how|which)\s+(do|does|did)\s+\w+\s+\w+.*\?$/i

    // Pattern 2: WH word + is/am/are/was/were + subject + ?
    const beVerbPattern = /^(what|where|when|why|who|how|which)\s+(is|am|are|was|were)\s+.*\?$/i

    return auxiliaryPattern.test(normalizedQuestion) || beVerbPattern.test(normalizedQuestion)
  }
  ...
  const getMotivationalMessage = (score: number): string => {
    if (score === 5) return "¡Excelente! Dominas perfectamente las preguntas WH en inglés."
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-gray-100 flex items-center justify-center p-4">
          ...
          {[...Array(50)].map((_, i) => (
            <motion.div
              ...
              initial={{
                ...
                backgroundColor: ["#FFD700", "#FF6347", "#7FFFD4", "#9370DB", "#20B2AA"][Math.floor(Math.random() * 5)],
              }}
              animate={{
                top: "100%",
      ...
      >
        <Card className="bg-gray-800 border-gray-700 shadow-xl overflow-hidden">
          ...
          <CardHeader className="relative border-b border-gray-700 bg-gray-850 pb-6">
            ...
            >
              ...
              <CardDescription className="text-gray-300 text-center mt-3">
                <div className="mb-2">
                  Escribe 5 preguntas en inglés del tipo WH siguiendo alguno de estos formatos:
                </div>
                <div className="space-y-1 text-sm bg-gray-900/50 p-3 rounded-lg border border-gray-700 mt-2">
                  <div className="font-mono text-blue-400">WH QUESTION + do/does/did + sujeto + verbo base ?</div>
                  <div className="font-mono text-purple-400">WH QUESTION + is/am/are/was/were + sujeto ?</div>
                </div>
                ...
                {questions.map((question, index) => (
                  ...
                  >
                    ...
                    {submitted && !results[index] && (
                      ...
                      >
                        Formato incorrecto. Recuerda: WH + do/does/did + sujeto + verbo base ? o WH + is/am/are/was/were
                        + sujeto ? La primera letra debe ser mayúscula.
                      </motion.p>
                    )}
                  </motion.div>
                ))}
                  ...
                  <div className="flex items-center">
                    <div
                      className={`w-12 h-12 rounded-full flex items-center justify-center mr-4 ${
                        score >= 3 ? "bg-green-500/20 text-green-400" : "bg-amber-500/20 text-amber-400"
                      }`}
                    >
                      <span className="text-xl font-bold">{score}</span>
                    </div>
                    <div>
                      <AlertTitle className="text-lg font-bold mb-1">Tu calificación: {score}/5</AlertTitle>
                      <AlertDescription className="text-base">{getMotivationalMessage(score)}</AlertDescription>
                    </div>
                  </div>
                </Alert>
              </motion.div>
          ...
          <CardFooter className="text-sm text-center text-gray-400 border-t border-gray-700 mt-6 bg-gray-850">
            <div className="w-full py-3">
              <p>Proyecto de Autómatas Gramaticales - Mayo 2025</p>
              <p className="mt-1">Tema: Aplicación para Validación de Preguntas en Inglés del Tipo WH</p>
            </div>
          </CardFooter>
        </Card>
      </motion.div>
